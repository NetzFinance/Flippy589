    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(
          "https://pro-api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=usd", //ids=ethereum
          options,
        );

        const data = await response.json();
        setData(data?.ethereum?.usd);
      } catch (err) {
        console.error(err);
